...
static void Hash(const char* input, char (&output)[33])
{
	...
	while (cin >> lengths[length_count]) ++length_count;
	...
	for (int i = 0; i < hash_count; i++)
	{
		...
		cout << std::setfill('0') << std::setw(2) << std::hex << hash;
	}

	strcpy_s(output, cout.str().c_str());
}